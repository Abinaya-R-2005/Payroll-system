...
const Register = () => {
  ...
  const [formData, setFormData] = useState({
    fullName: "", dob: "", personalEmail: "", phone: "", address: "",
    taxStatus: "Single", panNumber: "", aadharNumber: "", accountName: "",
    accountNumber: "", bankCode: "", accountType: "Savings", jobTitle: "",
    department: "", joiningDate: "", workLocation: "", emergencyName: "",
    emergencyRel: "", emergencyPhone: "", consent: false, email: "",
    password: "", confirmPassword: "", profilePhoto: null, profilePhotoPreview: null
  });
  ...
  const handlePhotoUpload = async (e) => {
    ...
    try {
      ...
      if (detections.length === 0) {
        setErrors(prev => ({ ...prev, profilePhoto: "No face detected in the image." }));
      }
    } catch (err) {
  ...
  const validateForm = () => {
    ...
    if (!formData.password) {
      ...
      else if (!/^[A-Z]/.test(password)) newErrors.password = "Password must start with a capital letter.";
  ...
  const handleSubmit = async (e) => {
    ...
    if (faceDetected === false) {
      alert("Please upload a photo where a face is clearly visible.");
      return;
    }
    setLoading(true);
    try {
      ...
      const res = await fetch("http://192.168.1.7:5001/register", {
        ...
        body: JSON.stringify({ ...formData, profilePhoto: photoBase64, role }),
      });
      ...
      navigate("/employee-success", { state: { fullName: data.employee.fullName, email: data.employee.email, employeeId: data.employee.employeeId } });
    } catch (error) {
  ...
  const ErrorMessage = ({ field }) => errors[field] && <p style={{ color: "#ef4444", fontSize: "0.85rem", margin: "5px 0 0 0" }}>{errors[field]}</p>;
        ...
        <div className="register-card">
          ...
          <form onSubmit={handleSubmit}>
            {Object.keys(errors).length > 0 && (
              <div style={{ background: "rgba(239, 68, 68, 0.15)", border: "1px solid rgba(239, 68, 68, 0.3)", color: "#fca5a5", padding: "16px", borderRadius: "12px", marginBottom: "25px", backdropFilter: "blur(10px)" }}>
                <p style={{ margin: "0 0 12px 0", fontWeight: "700", fontSize: "0.95rem", color: "#fecaca" }}>⚠️ Please fix these errors:</p>
                <ul style={{ margin: 0, paddingLeft: "20px" }}>
                  {Object.entries(errors).map(([field, message]) => (
                    <li key={field} style={{ margin: "6px 0", fontSize: "0.9rem" }}><strong>{field}:</strong> {message}</li>
                  ))}
                </ul>
              </div>
            )}
            ...
            <div className="form-section">
              ...
              {formData.accountName && (
                <p style={{ color: "#10b981", marginTop: "10px", fontWeight: "600", textAlign: "center" }}>✓ Account Holder: {formData.accountName}</p>
              )}
            ...
            <div className="form-section">
              ...
              <div style={{ marginBottom: "15px" }}>
                ...
                <div onClick={() => fileInputRef.current?.click()} style={{ border: "2px dashed #00d4ff", borderRadius: "12px", padding: "40px 20px", textAlign: "center", cursor: "pointer", background: "rgba(0, 212, 255, 0.08)", transition: "all 0.3s", marginBottom: "15px" }}>
              ...
              {formData.profilePhotoPreview && (
                <div style={{ marginBottom: "15px" }}>
                  <img src={formData.profilePhotoPreview} alt="Preview" style={{ maxWidth: "150px", maxHeight: "150px", borderRadius: "12px", border: "2px solid var(--primary)" }} />
                  <p style={{ marginTop: "10px", fontSize: "0.9rem", color: faceDetected ? "#10b981" : "#ef4444", fontWeight: "600" }}>
                    {faceDetected === null ? "Scanning..." : faceDetected ? "✓ Face detected" : "✗ Face not detected"}
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="form-section">
              ...
              <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', margin: '15px 0', padding: '8px 12px', background: 'rgba(255, 255, 255, 0.05)', borderRadius: '8px', width: 'fit-content' }}>
                <input type="checkbox" name="consent" checked={formData.consent} onChange={handleChange} required style={{ width: 'auto', margin: 0 }} />
                <span style={{ fontSize: '0.9rem', color: 'rgba(255, 255, 255, 0.9)', fontWeight: '500' }}>I agree to the <span style={{ color: '#00d4ff', textDecoration: 'underline' }}>Terms & Policy</span></span>
              </label>
            </div>
            ...
            <div className="form-section">
              ...
              <div className="form-section-grid">
                <div><Input label="Login Email" name="email" value={formData.email} onChange={handleChange} placeholder="name@fly.gmail.com" required /><ErrorMessage field="email" /></div>
                <div><Input label="Password" type="password" name="password" value={formData.password} onChange={handleChange} required /><ErrorMessage field="password" /></div>
                <div><Input label="Confirm Password" type="password" name="confirmPassword" value={formData.confirmPassword} onChange={handleChange} required /><ErrorMessage field="confirmPassword" /></div>
              </div>
            </div>
            ...
            <div style={{ textAlign: "center", marginTop: "1.5rem" }}>
              <p style={{ color: "#a0a0b0", margin: 0 }}>Already have an account? <Link to={`/login?role=${role}`} style={{ color: "#00d4ff", fontWeight: 600, textDecoration: 'none' }}>Login</Link></p>
